...
const ProductDAO = {
  ...
  async selectBySkipLimit(skip, limit) {
    ...
    const products = await Models.Product.find(query).sort(sortByDate).skip(skip).limit(limit).exec();
    return products;
  },
  async selectAll() {
  ...
  async selectTopHot(top) {
    ...
    console.log('Aggregated items:', items); // Log to see the result of aggregation
